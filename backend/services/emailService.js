...
/**
 * Main Entry Point: Sends Invoice Email with Signed PDF Attachment via Resend HTTPS API
 */
const sendInvoiceEmail = async ({ to, customerName, invoiceNumber, invoice, pdfBuffer }) => {
  ...
  // 3. Validate and Normalize PDF Buffer
  if (!pdfBuffer) {
  ...
  // Convert validated PDF Buffer to Base64 exactly once for Resend attachment
...
${businessEmail}
  ...
  const fromAddress = (process.env.EMAIL_FROM || `${businessName} <onboarding@resend.dev>`).trim();

  console.log(`[EmailService] Sending via Resend HTTPS API`);

  // 5. Send via Resend HTTPS API with Base64 PDF attachment
  ...
  console.log(`[EmailService] Resend email request sent`);
  ...
  console.log(`[EmailService] Invoice email sent successfully via Resend. ID: ${data?.id || "OK"}`);
  return data;
};
