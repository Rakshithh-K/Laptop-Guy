...
const sendViaResend = async ({
  ...
  if (!apiKey) {
    throw new Error(
      "RESEND_API_KEY is missing. Please add RESEND_API_KEY to your Render Dashboard Environment Variables."
    );
  }

  if (!Buffer.isBuffer(pdfBuffer)) {
    throw new Error("PDF attachment is not a valid Buffer.");
  }
  ...
  console.log(
    `[EmailService] Delivering via Resend HTTPS API to: ${to} (From: ${fromAddress})`
  );

  console.log(
    `[EmailService] PDF buffer size: ${pdfBuffer.length} bytes`
  );

  console.log(
    `[EmailService] PDF header: ${pdfBuffer.subarray(0, 5).toString()}`
  );
  ...
  console.log(
    `[EmailService] PDF Base64 generated (${pdfBase64.length} characters)`
  );
  ...
  if (error) {
    ...
    throw new Error(
      `Resend Delivery Error: ${errorDetail}`
    );
  }

  console.log(
    `[EmailService] Email sent successfully via Resend API to ${to}. ID: ${data?.id || "OK"}`
  );
...
/**
 * Main Entry Point
 * Sends Invoice Email with Signed PDF Attachment
 */
const sendInvoiceEmail = async ({
  ...
  if (!pdfBuffer || pdfBuffer.length === 0) {
    throw new Error(
      "PDF buffer is empty or invalid. Cannot send invoice email without attached PDF."
    );
  }
  ...
  console.log(
    `[EmailService] PDF attachment ready (${pdfBuffer.length} bytes)`
  );
   ...
   *
   * Supports:
   * 1. Multi-product invoice
   * 2. Older single-laptop invoice
   */
  const items =
    invoice.items &&
  ...
  /**
   * Generate HTML rows for all purchased products
   */
